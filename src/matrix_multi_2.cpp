/**
 * matrix_mult_2.cpp -- Simple program that multiplies two matrices
 *                      For simplicity, the matrices are square matrices and
 *                      threads are not being used for the task.
 *
 * NOTE: Increase the N_DIMS value for more computations.
 *       Higher values are preferred as you'll get to see the results in VTune
 *       more precisely. This will depend on what processor your system has, so
 *       tweak accordingly. As long as it takes kind of 10+ seconds, you're good
 *       to go.
 *
 */
